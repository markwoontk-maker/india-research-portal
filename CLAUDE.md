...
- **MF Category Flows** card (`#mfCatBody`, `loadMfCategories()`/`mfCatRender()`, below the MF Sector card on the Positioning tab). **Clean official AMFI data — no disclaimer.** Net inflows + AUM for the 6 equity cap categories (Large Cap, Large & Mid Cap, Mid Cap, Small Cap, Multi Cap, Flexi Cap). Sortable table (Category · net flow this-mo · prev · YTD · 1Y · AUM · Share%; YTD/1Y derived from `hist`) + click-a-name inline monthly net-flow bar chart (`mfCatChart`/`mfCatHistDraw`, reuses `secMonLbl`/`secCr`). Data **`data/mf_categories.json`**: `{asOf, prevAsOf, months[], categories:[{name, flow, flowPrev, aum, aumShare, hist[]}]}`. **`months`/`hist` are APPEND-ONLY — never trimmed** (the chart grows a bar each month; `mfCatOneY` uses `slice(-12)`, `mfCatYtd` filters by current year). Refreshed monthly (17th) by the Positioning Data Refresher from AMFI (finnovate fallback); `null` for unsourced cells.
